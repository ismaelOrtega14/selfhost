...
- Sin secrets de Swarm — todo pasa por `env_file: stack.env`
...
| 3     | `transmission`, `sonarr`, `radarr`, `jackett`, `bazarr`, `flaresolverr`, `seerr` |
| 4     | `yamtrack`, `redis-yamtrack`                             |
| 5     | `linkwarden`, `meilisearch`, `actual-budget`, `immich`, `shepherd`, `mealie`, `n8n`, `paperless` |
...
- Immich usa variables `${IMMICH_VERSION}`, `${DB_PASSWORD}`, `${UPLOAD_LOCATION}`, etc. desde stack.env
